...
export function stripHeavyFields(data: unknown, onStrip: (v: boolean) => void): unknown {
  ...
  if (Array.isArray(data) && data.length > 5) {
    ...
    if (sample.every((item) => item && typeof item === "object" && !Array.isArray(item))) {
      const heavyFields = findHeavyFields(sample as Array<Record<string, unknown>>);
...
function findHeavyFields(sample: Array<Record<string, unknown>>): string[] {
...
function stripFields(item: unknown, heavyFields: string[]): unknown {
...
function shrinkTopLevelArrays(obj: Record<string, unknown>): void {
  for (const [key, value] of Object.entries(obj)) {
